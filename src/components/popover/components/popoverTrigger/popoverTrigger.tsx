...
export function PopoverTrigger({children, offset = 8, position = "center", className}: PopoverTriggerProps) {
    const {id} = usePopover();
        ...
        if (trigger && popover) {
            ...
            const mutationObserver = new MutationObserver((mutations) => {
                mutations.forEach((mutation) => {
                    if (mutation.type === 'attributes' && mutation.attributeName === 'popover') {
                        updatePosition();
                    }
                });
            });
